...
def test_join_master_tables(spark):
    """
    Positive test that the function _join_master_tables returns the expected DataFrame.  # noqa: E501
    """
    ...
    customers_master_test = spark.createDataFrame(customer_master_test_data, schema=customer_schema)  # noqa: E501
    ...
    product_subcateogry_schema = st.StructType([
        st.StructField("ProductSubCategoryKey", st.IntegerType(), True),
    ...
    product_subcategory_master_test = spark.createDataFrame(
        product_subcategory_master_test_data,
        schema=product_subcateogry_schema
    )
